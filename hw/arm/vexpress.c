...
/************************************************/
    // 创建自定义UFS块设备
    DeviceState *ufsblk_dev;
    SysBusDevice *ufsblk_sbd;
...
/************************************************/
    ...
    ufsblk_dev = qdev_new(TYPE_UFSBLK);
    sysbus_realize_and_unref(SYS_BUS_DEVICE(ufsblk_dev), &error_fatal);
    ufsblk_sbd = SYS_BUS_DEVICE(ufsblk_dev);
    qemu_printf("ufsblk_sbd->num_mmio = %x\n", ufsblk_sbd->num_mmio);
    sysbus_mmio_map(ufsblk_sbd, 0, 0x20000000);
