...
static bool ufs_check_constraints(UfsHc *u)
{
    if (u->params.nutrs > UFS_MAX_NUTRS) {
        printf("nutrs must be less than or equal to %d",
                   UFS_MAX_NUTRS);
        return false;
    }

    if (u->params.nutmrs > UFS_MAX_NUTMRS) {
        printf("nutmrs must be less than or equal to %d",
                   UFS_MAX_NUTMRS);
        return false;
    }

    if (u->params.mcq_maxq >= UFS_MAX_MCQ_QNUM) {
        printf("mcq-maxq must be less than %d", UFS_MAX_MCQ_QNUM);
        return false;
    }
...
{
    // 初始化 MMIO 内存区域（和原逻辑一致，但直接注册到系统总线）
    memory_region_init_io(&u->iomem, OBJECT(u), &ufs_mmio_ops, u, "ufs", u->reg_size);
    // 将 MMIO 区域映射到系统总线（CPU 可直接访问）
    sysbus_init_mmio(sbd, &u->iomem);
    // 初始化中断（直连 CPU 中断控制器，需适配具体平台）
    // u->irq = g_malloc0(sizeof(qemu_irq));
...
static void ufs_realize(SysBusDevice *sbd)
{
    UfsHc *u = UFS(sbd);

    if (!ufs_check_constraints(u)) {
        return;
    }
    ...
    ufs_init_sysbus(u, sbd);  // 替换为系统总线初始化
...
static void ufs_class_init(ObjectClass *oc, const void *data)
{
    ...
    // SysBusDeviceClass *sbc = SYS_BUS_DEVICE_CLASS(oc);

    printf("enter in ufs_class_init\n");
...
static void ufs4_init(Object *obj)
{
    SysBusDevice *dev = SYS_BUS_DEVICE(obj);

    printf("enter in ufs4_init\n");
    ufs_realize(dev);
}
...
static void ufs_bus_class_init(ObjectClass *class, const void *data)
{
    ...
    printf("enter in ufs_bus_class_init\n");
